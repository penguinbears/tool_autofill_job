...
- 候选人档案、页面扫描结果、诊断文件、投递历史导出和本地密钥均被忽略。
- 扩展只在用户操作后临时访问当前页面，目前不申请持续读取所有网站的权限。
- 不填写密码、文件选择器、证件号、银行卡等敏感字段。
- 诊断 HTML 会移除脚本、表单输入值、查询参数以及疑似令牌属性；分享前仍应人工检查页面可见文本。
- 不自动点击最终提交按钮。
...
遇到不兼容的网站时，优先提供扩展生成的诊断文件以及对应控件截图。诊断信息不足时，再提供相关区域的脱敏 HTML。
