...
- 自动识别招聘网站顶部账户区域的登录状态，并可逐站开启 Edge 后台登录保活。
...
1. 登录招聘网站后打开插件，等待“登录保活”识别顶部账户区域。
2. 状态显示“已登录”后，点击“开启当前网站”并确认该网站的运行时权限。
3. Edge 会在后台每 10 分钟向当前页面地址发送一次不带查询参数的只读请求，不打开标签页或刷新页面。
4. 可随时在相同网站打开插件并点击“停用当前网站”。锁屏且电脑仍处于唤醒状态时可以继续运行；睡眠、休眠、断网或明确退出 Edge 时不能保活。

保活只能延长支持滑动过期的服务器会话；网站设置绝对登录时限、验证码或风控重新认证时，仍需用户手动登录。
...
- 页面扫描和填写只在用户操作后访问当前页面；登录保活仅对用户逐站确认授权的网站执行后台请求。
...
- 不自动点击最终提交按钮。
- 登录保活不读取、导出或保存 Cookie 值，只让 Edge 在授权域名的只读请求中自行携带现有凭证。
...
- 电脑睡眠、休眠、断网或退出 Edge 时，后台登录保活不会运行；绝对会话超时也无法通过定时请求绕过。
