...
| `UI-ACTIONS` | 扫描与填充操作区 | 弹窗顶部“扫描当前页 / 填充已匹配项 / 未匹配网页诊断” | 发起扫描、填写和覆盖已有值 | `popup.html`、`popup.js` |
...
| `UI-DIAGNOSTIC` | 未匹配网页诊断 | 扫描或填充后点击弹窗顶部“未匹配网页诊断” | 导出当前网页的实时 HTML 结构，移除脚本、表单值和 URL 查询参数 | `content.js` 的 `pageHtmlSnapshot()`、`popup.js` 的 `exportUnmatchedPageHtml()` |
...
4. 插件导出的 `job-autofill-diagnostic-*.json`。
5. 展开后的下拉框或目标区段截图。
6. 诊断仍不足时，再提供保存的 HTML。
